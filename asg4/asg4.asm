...
//Memory Allocation 
	pyra_s = 16					//pyramid(another name for "first_s") offset	
...
//initialization	
	mov	w9, 0
	str	w9, [x29, pyra_origin + point_x]	//p.origin.x = 0
	ldr 	w9, [x29, pyra_origin + point_x]
	str 	w9, [x8, pyra_origin + point_x]

	mov	w9, 0
	str	w9, [x29, pyra_origin + point_y]	//p.origin.y = 0
	ldr 	w9, [x29, pyra_origin + point_y]
	str 	w9, [x8, pyra_origin + point_y]
	
	mov	w10, 2						//w9 = 2
	str	w10, [x29, pyra_base + dim_width]	//w9 = p.base.width
	ldr 	w9, [x29, pyra_base + dim_width]
	str 	w9, [x8, pyra_base + dim_width]
	
	mov	w11, 2						//w10 = 2
	str	w11, [x29, pyra_base + dim_length]	//w10 = p.base.length
	ldr 	w9, [x29, pyra_base + dim_length]
	str 	w9, [x8, pyra_base + dim_length]
	
	mov	w12, 3						//w11 = 3
	str	w12, [x29, pyra_height]		//w11 = p.height
	ldr 	w9, [x29, pyra_height]
	str 	w9, [x8, pyra_height]
	
	mul	w12, w9, w10					//w12 = p.base.width * p.base.length
	mul	w12, w12, w11					//w12 = w12 * p.height
	mov	w13, 3 						//w13 = 3
	sdiv	w12, w12, w13		    			//w12 = w12 / w13
	str	w12, [x29, pyra_volume]		//p.volume = (p.base.width * p.base.length * p.height) / 3
	ldr	w9, [x29, pyra_volume]
	str	w9, [x8, pyra_volume]
...
main:	stp	x29, x30, [sp, alloc]!
	...
	cmp	w0, false					//if (!equalSize(&first, &second))
...
move:	stp	x29, x30, [sp, -16]!
	...
	ldr	w9, [x0, pyra_origin + point_x]			//p->origin.x 
	add	w9, w9, w1					//p->origin.x += deltaX
	str	w9, [x0, pyra_origin + point_x]
...
printPyramid:
	...
	ldr	w9, [x1, pyra_origin + point_x]	//p->origin.x
	ldr	w10, [x1, pyra_origin + point_y]	//p->origin.y
	ldr	w11, [x1, pyra_base + dim_width]	//p->base.width
	ldr	w12, [x1, pyra_base + dim_length]	//p->base.length
	ldr	w13, [x1, pyra_height]			//p->height
	ldr	w14, [x1, pyra_volume]			//p->volume
	adrp	x0, debug
	add	x0, x0, :lo12:debug
	bl	printf						//print initial pyramid
	...
	adrp	x0, pyra
	add	x0, x0, :lo12:pyra		
	mov	w2, w9						//print p->origin.x
	mov	w3, w10						//print p->origin.y
	bl	printf				
